...
- Отображение информации о таблицах (количество строк, размер, тип движка)
- Выбор одной или нескольких таблиц для экспорта
- Экспорт выбранных таблиц в SQL-формат
- Отображение прогресса и времени экспорта
- Сохранение настроек подключения между запусками
- Запоминание последней папки для экспорта

### 1.3 Системные требования

- **Операционная система**: Windows 10/11, Linux (Ubuntu 20.04+, Debian 11+), macOS 11+
...
3. **Имя пользователя** — логин для доступа к базе данных
4. **Пароль** — пароль пользователя
5. **База данных** — имя базы данных для подключения
...
При успешном подключении параметры автоматически сохраняются (кроме пароля из соображений безопасности). При следующем запуске программы:
- Поля будут автоматически заполнены сохраненными значениями
- Необходимо будет только ввести пароль и нажать "Подключиться"
...
**[СКРИНШОТ 5: Главное окно программы с пронумерованными элементами]**
...
7. **Кнопка "Сменить подключение"** — для подключения к другой базе данных
...
| **Строк** | Количество записей в таблице |
| **Размер** | Размер таблицы (автоматически в B, KB, MB или GB) |
| **Тип движка** | Тип движка базы данных (InnoDB, MyISAM и др.) |
...
- Или нажмите кнопку очистки (✕) в правой части поля поиска
...
**Способ 1:** Кликните на чекбокс в заголовке таблицы

**Способ 2:** Нажмите кнопку **"Выбрать все"** внизу окна
...
- Нажмите кнопку **"Снять выбор"** внизу окна
- Или кликните на чекбокс в заголовке таблицы (если все таблицы выбраны)
...
- Строка состояния с текущим действием и затраченным временем
- Например: `Экспорт таблицы 'users' (5234 строк)... 00:00:15`
...
**[СКРИНШОТ 15: Сообщение об ошибке экспорта]**
...
1. Нажмите кнопку **"Сменить подключение"** в нижней части главного окна
...
**macOS:**
- Разрешите запуск приложения в Настройках → Безопасность
- Для первого запуска: Ctrl+клик → Открыть
...
| Очистить поиск | `Esc` (в поле поиска) |
| Выбрать все | `Ctrl + A` |
| Закрыть окно | `Alt + F4` (Windows), `Cmd + Q` (macOS) |

### Приложение Б: Размеры файлов

Программа автоматически отображает размеры в удобочитаемом формате:

| Диапазон | Формат |
|----------|--------|
| < 1 KB | Байты (B) |
| 1 KB - 1 MB | Килобайты (KB) |
| 1 MB - 1 GB | Мегабайты (MB) |
| ≥ 1 GB | Гигабайты (GB) |

### Приложение В: Формат даты и времени

В именах файлов используется формат: `ГГГГММДД_ЧЧММСС`
- Пример: `20241121_143052` = 21 ноября 2024 года, 14:30:52
