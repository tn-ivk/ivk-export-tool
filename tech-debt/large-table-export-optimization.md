...
### Этап 1: Критичные исправления (MUST HAVE)
- [ ] Переписать `ExportTableDataAsync` с потоковой записью (п. 1.1)
- [ ] Внедрить `StringBuilder` для экранирования строк (п. 1.2)
- [ ] Добавить unit-тесты для проверки памяти (мокировать большие таблицы)
