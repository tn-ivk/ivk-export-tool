...
### Этап 2: Улучшение UX (SHOULD HAVE) ✅ ВЫПОЛНЕНО
- [x] Добавить модель `ExportProgress` (п. 2.1)
- [x] Обновить интерфейс `IExportService` для поддержки детального прогресса
- [x] Обновить `MainWindowViewModel` для отображения детального прогресса
- [x] Добавить отображение количества строк в UI
...
**Реализованные изменения**:
1. Создана модель `ExportProgress` в `src/IvkExportTool.Core/Models/ExportProgress.cs` с детальной информацией:
   - Имя текущей таблицы
   - Количество обработанных строк
   - Индекс текущей таблицы и общее количество таблиц
   - Процент выполнения
   - Прошедшее время
   - Текстовое сообщение о статусе

2. Обновлен интерфейс `IExportService` - добавлен параметр `IProgress<ExportProgress>? detailedProgress`

3. Обновлен `SqlExportService` для отчета о детальном прогрессе:
   - Отчет каждые 10000 обработанных строк
   - Финальный отчет по завершении таблицы
   - Метод `CalculatePercent` для вычисления общего прогресса

4. Обновлен `MainWindowViewModel`:
   - Добавлены свойства: `CurrentTableName`, `CurrentTableRowsProcessed`, `DetailedStatusMessage`, `ElapsedTime`
   - Добавлен обработчик `detailedProgress` для обновления UI в реальном времени
   - Метод `FormatElapsedTime` для форматирования времени

5. Обновлен UI в `MainWindow.axaml`:
   - Добавлена секция детального прогресса в статус-баре (показывается только во время экспорта)
   - Отображение текущей таблицы и количества обработанных строк
   - Progress bar с процентом выполнения
   - Счетчик прошедшего времени (HH:MM:SS)
